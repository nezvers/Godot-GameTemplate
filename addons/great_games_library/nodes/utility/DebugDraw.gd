...
static func debug_line2d(from:Vector2, to:Vector2, color:Color, time:float)->void:
	if singleton == null:
		return
...
static func debug_distance2d(from:Vector2, distance:Vector2, color:Color, time:float)->void:
	if singleton == null:
		return
